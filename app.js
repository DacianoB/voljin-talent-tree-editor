...
let stateBase=clone(base),state=clone(base),catalogPromise,catalogData,editing=false,refunding=false,selected=null,connectFrom=null,zoom=1,dirty=false,dragMoved=false;
...
const loadCatalog=()=>catalogPromise??=fetch("data/voljin.json",{cache:"no-cache"}).then(async response=>{if(!response.ok)throw new Error("The Ascension catalog is unavailable.");return catalogData=await response.json()});
...
function render(){renderIdentity();renderBrowser();renderTabs();renderTree();renderInspector();updateStatus()}
function renderIdentity(){const meta=state.meta||base.meta;$("builder-subtitle").textContent=`Vol'jin · ${meta.className} / ${meta.specName}`;$("source-current").textContent=`${meta.className} / ${meta.specName}`;$('class-name').textContent=meta.className;$('spec-name').textContent=meta.specName.toUpperCase();$('spec-button').textContent=meta.specName;const image=state.trees.class.nodes[0]?.image;$('class-icon').style.cssText=image?`background-image:url('${image}');background-size:cover;background-position:center`:"background-position:40.7407% 72.2222%";$('tree-stage').style.setProperty("--tree-bg",`url('https://ascension.gg/textures/coa/trees/${meta.slug}.webp')`)}
function fillSpecs(classId,selectedId){const tabs=catalogData.classes.find(item=>item.id===+classId)?.tabs.filter(tab=>tab.id!==87)||[];$("spec-select").innerHTML=tabs.map(tab=>`<option value="${tab.id}">${escapeHtml(tab.name)}</option>`).join("");$("spec-select").value=String(selectedId||tabs[0]?.id||"");$("spec-select").dataset.classId=classId}
function renderBrowser(){if(!catalogData)return;const fallbackClass=catalogData.classes.find(item=>item.name===base.meta.className)||catalogData.classes[0],fallbackSpec=fallbackClass.tabs.find(tab=>tab.name===base.meta.specName&&tab.id!==87)||fallbackClass.tabs.find(tab=>tab.id!==87),selection=state.catalog||{classId:fallbackClass.id,specTabId:fallbackSpec.id},classSelect=$("class-select");classSelect.value=selection.classId;if($("spec-select").dataset.classId!==String(selection.classId))fillSpecs(selection.classId,selection.specTabId);else $("spec-select").value=selection.specTabId}
async function initTreeBrowser(){try{await loadCatalog();$("class-select").innerHTML=catalogData.classes.map(item=>`<option value="${item.id}">${escapeHtml(item.name)}</option>`).join("");renderBrowser();$("open-tree-btn").disabled=false;$("browser-state").textContent=`${catalogData.classes.length} classes available.`}catch(error){$("browser-state").textContent=error.message}}
function openTree(next,pristine){stateBase=pristine;state=next;editing=false;refunding=false;selected=null;connectFrom=null;$("edit-toggle").checked=false;$("tree-source").open=false;history.replaceState(null,"",location.pathname+location.search);markDirty();render()}
function renderTabs(){
...
function renderTree(){
  ...
  $("nodes").innerHTML=t.nodes.map(n=>{
    ...
    const thresholdLocked=!meetsThreshold(n,spent),flowLocked=!n.passive&&!hasActiveUpperConnection(n,t.nodes,t.edges),locked=thresholdLocked||flowLocked,available=canSelectNode(n,t.nodes,t.edges,spent,t.budget);
...
$("class-select").onchange=event=>{fillSpecs(event.target.value);$("browser-state").textContent="Choose a specialization, then open the tree."};
$("tree-browser").onsubmit=event=>{event.preventDefault();if(dirty&&!confirm("Open another tree and discard unsaved changes?"))return;const selection={classId:+$("class-select").value,specTabId:+$("spec-select").value},pristine=catalogBase(catalogData,selection),next=clone(pristine);next.active="spec";openTree(next,pristine);$("browser-state").textContent=`Opened ${next.meta.className} / ${next.meta.specName}.`;toast("Premade tree opened.")};
$("ascension-import").onsubmit=async event=>{event.preventDefault();if(dirty&&!confirm("Import another build and discard unsaved changes?"))return;const button=$("import-btn"),status=$("import-state");button.disabled=true;status.textContent="Loading the scheduled Ascension catalog…";try{const catalog=await loadCatalog(),imported=catalogState(catalog,await decodeAscensionBuild($("ascension-url").value));openTree(imported,catalogBase(catalog,imported.catalog));status.textContent=`Loaded ${state.meta.className} / ${state.meta.specName}.`;toast("Ascension build imported.")}catch(error){console.warn("Could not import Ascension build",error);status.textContent=error.message;toast("Ascension build could not be imported.")}finally{button.disabled=false}};
...
await loadInitial();render();await initTreeBrowser();
}
